...
func (s *testPlanSuite) TestDAGPlanBuilderSimpleCase(c *C) {
	...
	for i, tt := range input {
		...
		c.Assert(err, IsNil)
		p, _, err := planner.Optimize(context.TODO(), se, stmt, s.is, false)
		c.Assert(err, IsNil)
...
func (s *testPlanSuite) TestUnmatchedTableInHint(c *C) {
	...
	for i, test := range input {
		...
		c.Assert(err, IsNil)
		_, _, err = planner.Optimize(context.TODO(), se, stmt, s.is, false)
		c.Assert(err, IsNil)
		warnings := se.GetSessionVars().StmtCtx.GetWarnings()
		s.testData.OnRecord(func() {
			output[i].SQL = test
...
func (s *testPlanSuite) TestHintScope(c *C) {
	...
	for i, test := range input {
		...
		p, _, err := planner.Optimize(context.Background(), se, stmt, s.is, false)
		c.Assert(err, IsNil)
		s.testData.OnRecord(func() {
			output[i].SQL = test
			output[i].Best = core.ToString(p)
...
func (s *testPlanSuite) TestJoinHints(c *C) {
	...
	for i, test := range input {
		...
		se.GetSessionVars().StmtCtx.SetWarnings(nil)
		p, _, err := planner.Optimize(ctx, se, stmt, s.is, false)
		c.Assert(err, IsNil)
		warnings := se.GetSessionVars().StmtCtx.GetWarnings()
...
func (s *testPlanSuite) TestAggToCopHint(c *C) {
	...
	for i, test := range input {
		...
		p, _, err := planner.Optimize(ctx, tk.Se, stmt, is, false)
		c.Assert(err, IsNil)
...
func (s *testPlanSuite) TestHintAlias(c *C) {
	...
	for i, tt := range tests {
		...
		p1, _, err := planner.Optimize(ctx, se, stmt1, s.is, false)
		c.Assert(err, IsNil)
		p2, _, err := planner.Optimize(ctx, se, stmt2, s.is, false)
		c.Assert(err, IsNil)
...
func (s *testPlanSuite) TestInlineProjection(c *C) {
	...
	for i, tt := range input {
		...
		p, _, err := planner.Optimize(ctx, se, stmt, is, false)
		c.Assert(err, IsNil, comment)
		s.testData.OnRecord(func() {
			output[i].SQL = tt
			output[i].Plan = core.ToString(p)
