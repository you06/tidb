...
func (s *testSnapshotSuite) checkAll(keys []kv.Key, c *C) {
	txn := s.beginTxn(c)
	snapshot := newTiKVSnapshot(s.store, kv.Version{Ver: txn.StartTS()}, 0, 0)
...
func (s *testSnapshotSuite) TestPointGetSkipTxnLock(c *C) {
	...
	snapshot := newTiKVSnapshot(s.store, kv.MaxVersion, 0, 0)
	start := time.Now()
...
func (s *testSnapshotSuite) TestSnapshotRuntimeStats(c *C) {
	...
	snapshot := newTiKVSnapshot(s.store, kv.Version{Ver: 0}, 0, 0)
