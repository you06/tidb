...
func TestTiDBUpgradeToVer256(t *testing.T) {
	...
	// Add back old columns to simulate the old schema
	seV255.SetValue(sessionctx.Initing, true)
	seV255.GetSessionVars().SQLMode = mysql.ModeNone
	mustExecute(seV255, "ALTER TABLE mysql.table_cache_meta ADD COLUMN lock_type enum('NONE','READ','INTEND','WRITE') NOT NULL DEFAULT 'NONE'")
	mustExecute(seV255, "ALTER TABLE mysql.table_cache_meta ADD COLUMN lease bigint(20) NOT NULL DEFAULT 0")
	mustExecute(seV255, "ALTER TABLE mysql.table_cache_meta ADD COLUMN oldReadLease bigint(20) NOT NULL DEFAULT 0")
	...
	createSQL := getTableCreateSQLFn(seV255, "table_cache_meta")
	require.Contains(t, createSQL, "lock_type")
	require.Contains(t, createSQL, "lease")
	require.Contains(t, createSQL, "oldReadLease")
	...
	// Verify old columns have been dropped after upgrade
	createSQL = getTableCreateSQLFn(seCurVer, "table_cache_meta")
	require.NotContains(t, createSQL, "lock_type")
	require.NotContains(t, createSQL, "lease")
	require.NotContains(t, createSQL, "oldReadLease")
	require.Contains(t, createSQL, "tid")
}
